...
class MyTeamController extends Controller
{
    ...
    public function store(Request $request) {
        ...
        if (count(Auth::user()->teams) >= 4) {
            return back()->withErrors(['You can not be a member of more than 4 teams.']);
        }
    ...
    public function update($teamID, Request $request) {
        ...
        if ($team->captain != Auth::user()->username) {
            return back()->withErrors(['Hold on, you are not the team Captain!!']);
        }
    ...
    public function kick($teamID, $userID) {
        ...
        if ($team->captain !== Auth::user()->username) {
            return response()->json([
                'result' => 'error',
                'message' => 'Hold on, you are not the team Captain!!'
            ]);
        }
